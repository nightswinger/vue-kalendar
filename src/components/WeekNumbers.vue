...
const props = defineProps<{ activeStartDate: Date }>()

const numberOfWeeks = computed(() => {
  const numberOfDays = getDaysInMonth(props.activeStartDate)
  const startWeekday = getDayOfWeek(props.activeStartDate)
...
const dates = computed(() => {
  ...
  for (let i = 0; i < numberOfWeeks.value; i += 1) {
    result.push(getBeginOfWeek(new Date(year, month, day + i * 7)))
  }
